...
function Renewals() {
  ...
  const renewalPagination = usePagination(renewals, { resetKey: `${query}|${range}` })
  ...
  return <section className="page-stack">
    ...
    <section className="panel"><div className="member-toolbar payment-toolbar"><div className="search-box"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search member, phone, or plan" /></div><select className="filter-select" value={range} onChange={(event) => setRange(event.target.value)}><option value="7">Next 7 days + expired</option><option value="30">Next 30 days + expired</option><option value="90">Next 90 days + expired</option><option value="3650">All dated memberships</option></select><button className="secondary-button" type="button" onClick={loadMembers}><RefreshCw size={16} /> Refresh</button></div>{error && <p className="dashboard-notice error" role="alert">{error}</p>}<div className="member-table-wrap"><table className="member-table renewal-table"><thead><tr><th>Member</th><th>Plan</th><th>End date</th><th>Renewal status</th><th>Member status</th><th>Actions</th></tr></thead><tbody>{renewalPagination.pageItems.map((member) => <tr key={member._id}><td data-label="Member"><strong>{member.name}</strong><span>{member.phone}</span></td><td data-label="Plan">{member.plan?.name || 'No plan'}</td><td data-label="End date">{new Date(member.membershipEnd).toLocaleDateString('en-IN')}</td><td data-label="Renewal status"><span className={`renewal-state ${member.daysRemaining < 0 ? 'overdue' : member.daysRemaining <= 7 ? 'urgent' : ''}`}>{member.daysRemaining < 0 ? `${Math.abs(member.daysRemaining)} days overdue` : member.daysRemaining === 0 ? 'Due today' : `${member.daysRemaining} days left`}</span></td><td data-label="Member status"><span className="status-pill">{member.status}</span></td><td data-label="Actions"><div className="table-actions"><button className="primary-button compact" type="button" onClick={() => setRenewingMember(member)}><RotateCw size={14} /> Renew</button><Link className="secondary-button compact" to={`/dashboard/members?search=${encodeURIComponent(member.phone)}`}>Open</Link></div></td></tr>)}</tbody></table></div><Pagination pagination={renewalPagination} label="renewals" />{renewals.length === 0 && <p className="empty-state">No renewals match this range.</p>}</section>
    {renewingMember && <MemberModal member={renewingMember} renewal onClose={() => setRenewingMember(null)} onSaved={() => { setRenewingMember(null); loadMembers() }} />}
  </section>
}
