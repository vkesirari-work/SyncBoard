...
function Leads() {
  ...
  const leadPagination = useServerPagination(total, { pageSize: 48, resetKey: `${deferredQuery}|${statusFilter}` })

  const loadLeads = useCallback(async () => {
    try {
      const { data } = await api.get('/leads', { params: { page: leadPagination.page, limit: leadPagination.pageSize, q: deferredQuery || undefined, status: statusFilter === 'all' ? undefined : statusFilter } })
      setLeads(data.leads)
      setTotal(data.pagination?.total ?? data.leads.length)
      setCounts(data.counts || Object.fromEntries(pipelineColumns.map(({ id }) => [id, data.leads.filter((lead) => lead.status === id).length])))
      setError('')
    } catch (requestError) {
      setError(requestError.response?.data?.message || 'Could not load leads.')
    }
  }, [deferredQuery, leadPagination.page, leadPagination.pageSize, statusFilter])
  ...
  return (
    <section className="page-stack">
      ...
      <div className="stats-grid">
        {['new', 'contacted', 'converted', 'closed'].map((status) => <article className="stat-card" key={status}><UserRoundSearch size={20} /><strong>{counts[status] || 0}</strong><span className="capitalize">{status} leads</span></article>)}
      </div>
              ...
              <div className="lead-card-list">
                {columnLeads.slice(0, visibleCounts[column.id]).map((lead) => (
                  <article className={`lead-card ${draggedLeadId === lead._id ? 'is-dragging' : ''} ${movingId === lead._id ? 'is-moving' : ''}`} draggable={movingId !== lead._id} key={lead._id} onDragStart={(event) => startDragging(event, lead)} onDragEnd={() => { setDraggedLeadId(null); setDragOverColumn(null) }}>
                    ...
                    <div className="lead-card-move">
                      <span className="lead-drag-note"><GripVertical size={13} /> Drag to move</span>
                      <label>
                        <span>Move lead</span>
                        <select aria-label={`Move ${lead.name} to status`} value={lead.status} disabled={movingId === lead._id} onChange={(event) => moveLead(lead, event.target.value)}>
                          {pipelineColumns.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}
                        </select>
                      </label>
                    </div>
                  </article>
      ...
      <Pagination pagination={leadPagination} label="leads" />
