...
export async function deleteMember(request, response, next) {
  try {
    ...
    if (paymentCount || attendanceCount || linkedMember.userAccount) {
      return response.status(409).json({
        message: 'Member has payment, attendance, or login history and cannot be deleted. Mark the member expired instead.',
      })
    }
